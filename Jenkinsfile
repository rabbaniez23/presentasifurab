            ...
            steps {
                powershell '''
                    ...
                    foreach ($s in $services) {
                        ...
                        docker build --pull=false -t "$env:DOCKER_REGISTRY/$($s.Name):$env:BUILD_NUMBER" -t "$env:DOCKER_REGISTRY/$($s.Name):latest" -f "$($s.FullName)/Dockerfile" .
                        if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
                    }
                '''
            }
        ...
        stage('Push Image') {
            when {
                anyOf {
                    branch 'main'
                    branch 'origin/main'
                    expression { return env.BRANCH_NAME == 'main' || env.GIT_BRANCH == 'main' || env.GIT_BRANCH == 'origin/main' || env.GIT_BRANCH?.endsWith('/main') }
                }
            }
            steps {
                powershell '''
